...
public:
    ...
    int GetDocumentId(int index) const {
        ...
        throw out_of_range("document index out of range"s);
    }
...
private:
    ...
    QueryWord ParseQueryWord(string text) const {
        if (text.empty()) {
            throw invalid_argument("no query word"s);
        }
        bool is_minus = false;
        if (text[0] == '-') {
            is_minus = true;
        ...
        if (text.empty() || text[0] == '-') {
            throw invalid_argument("no query word after '-'"s);
        }
        if (!IsValidWord(text)) {
            throw invalid_argument("incorrect word"s);
        }
    ...
    Query ParseQuery(const string& text) const {
        ...
        for (const string& word : SplitIntoWords(text)) {
            ...
            if (query_word.data.empty()) {
                throw invalid_argument("no query words"s);
            }
